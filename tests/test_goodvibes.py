...
import pytest
import math
from goodvibes import constants, utils
from goodvibes.bbe import calc_bbe
...
@pytest.mark.parametrize(
    "path, QS, temp, E, ZPE, H, TS, TqhS, G, qhG",
    [
        # Grimme, 298.15K
        (
            "Al_298K.out",
...
def test_QS(path, QS, temp, E, ZPE, H, TS, TqhS, G, qhG):
    ...
    QH, s_freq_cutoff, h_freq_cutoff, freq_scale_factor, solv, spc, invert, d3 = (
        False,
        100.0,
        100.0,
        1.0,
...
@pytest.mark.parametrize(
    "path, QS, temp, E, ZPE, H, qhH, TS, TqhS, G, qhG",
    [
...
def test_temperature_corrections_QS(QS, E, ZPE, H, TS, TqhS, G, qhG):
    ...
    precision = 6
    assert (
        E == round(bbe298.scf_energy, precision) == round(bbe400.scf_energy, precision)
    )
    ...
    assert (
        TS
        == round(temp * bbe298.entropy, precision)
        == round(temp * bbe400.entropy, precision)
    )
    assert (
        TqhS
        == round(temp * bbe298.qh_entropy, precision)
        == round(temp * bbe400.qh_entropy, precision)
    )
    assert (
        G
        == round(bbe298.gibbs_free_energy, precision)
        == round(bbe400.gibbs_free_energy, precision)
    )
    assert (
        qhG
        == round(bbe298.qh_gibbs_free_energy, precision)
        == round(bbe400.qh_gibbs_free_energy, precision)
    )


@pytest.mark.parametrize(
    "QS, E, ZPE, H, qhH, TS, TqhS, G, qhG",
    [
        (
            "grimme",
            -242.328708,
...
def test_temperature_corrections_QH(QS, E, ZPE, H, qhH, TS, TqhS, G, qhG):
    ...
    assert (
        qhH
        == round(bbe298.qh_enthalpy, precision)
        == round(bbe400.qh_enthalpy, precision)
    )
    assert (
        TS
        == round(temp * bbe298.entropy, precision)
...
@pytest.mark.parametrize(
    "spc, E_spc, E, ZPE, H, TS, TqhS, GT, qhGT",
    [
        (
            False,
            None,
...
@pytest.mark.parametrize(
    "path, ti, H, TS, TqhS, GT, qhGT",
    [
        (
            "allene.out",
            "200,300,40",
            [-116.512865, -116.512128, -116.511313],
            [0.016953, 0.021149, 0.025552],
            [0.016955, 0.021151, 0.025555],
            [-116.529818, -116.533277, -116.536865],
            [-116.529821, -116.533280, -116.536868],
        ),
        (
            "ethane.out",
            "200,300,40",
            [-79.752458, -79.751811, -79.751109],
            [0.017099, 0.021225, 0.025519],
            [0.017101, 0.021227, 0.025521],
            [-79.769556, -79.773036, -79.776628],
            [-79.769558, -79.773038, -79.776630],
        ),
        (
            "methylaniline.out",
            "200,300,40",
            [-326.518529, -326.51706, -326.515348],
            [0.023362, 0.029637, 0.036421],
            [0.023345, 0.029579, 0.036313],
            [-326.541891, -326.546698, -326.551769],
            [-326.541875, -326.546639, -326.551661],
        ),
    ],
)
def test_temperature_interval(path, ti, H, TS, TqhS, GT, qhGT):

    QS, QH, s_freq_cutoff, h_freq_cutoff, freq_scale_factor, solv, spc, invert, d3 = (
        "grimme",
        False,
        100.0,
        100.0,
...
@pytest.mark.parametrize(
    "filename, freq_scale_factor, zpe", [("ethane.out", 0.977, 0.073508)]
)
def test_scaling_factor_search(filename, freq_scale_factor, zpe):
    temp = 298.15
    conc = constants.ATMOS / (constants.GAS_CONSTANT * temp)
    QS, QH, s_freq_cutoff, h_freq_cutoff, solv, spc, invert, d3 = (
        "grimme",
        True,
        100.0,
        100.0,
...
@pytest.mark.parametrize(
    "path, conc, QS, E, ZPE, H, TS, TqhS, G, qhG",
    [
        # no c correction applied
        (
            "media_conc/Benzene.log",
            0,
            "grimme",
            -232.227201,
            0.101377,
...
def test_concentration_correction(path, conc, QS, E, ZPE, H, TS, TqhS, G, qhG):
    path = datapath(path)
    QH, s_freq_cutoff, h_freq_cutoff, freq_scale_factor, temp, solv, spc, invert, d3 = (
        False,
        100.0,
        100.0,
        1.0,
...
@pytest.mark.parametrize(
    "path, conc, media, E, ZPE, H, TS, TqhS, G, qhG",
    [
...
def test_media_correction(path, conc, media, E, ZPE, H, TS, TqhS, G, qhG):
    ...
    (
        ...
        invert,
        d3,
    ) = (False, "grimme", 100.0, 100.0, 1.0, 298.15, "none", False, False, 0)
    bbe = calc_bbe(
        path,
        QS,
        QH,
    ...
    if media is not False:
        ...
        media_correction = -(constants.GAS_CONSTANT / constants.J_TO_AU) * math.log(
    ...
    if hasattr(bbe, "gibbs_free_energy"):
        ...
        assert G == round(
            bbe.gibbs_free_energy + (temp * (-media_correction)), precision
        )
        assert qhG == round(
...
def test_pes(E, ZPE, H, TS, TqhS, GT, qhGT):
    ...
    precision = 2
    files = [
        "pes/Int-III_Oax_cis_a.log",
        "pes/Int-II_Oax_cis_a.log",
        "pes/Int-I_Oax.log",
        "pes/TolS.log",
        "pes/TolSH.log",
    ]
    files = [datapath(file) for file in files]
    log = utils.Logger("GoodVibes", "test", False)

    bbe_vals = []
    for (
        file
    ) in files:  # loop over all specified output files and compute thermochemistry
        bbe = calc_bbe(
            file,
            QS,
            QH,
    ...
    thermo_data = dict(
        zip(fileList, bbe_vals)
    )  # the collected thermochemical data for all files

    pes = get_pes(
        datapath("pes/Cis_complete_pathway.yaml"), thermo_data, log, temp, gconf, QH
    )
    ...
    zero_vals = [
        ...
        pes.zpe_zero[0][0],
        pes.h_zero[0][0],
        temp * pes.ts_zero[0][0],
        temp * pes.qhts_zero[0][0],
        pes.g_zero[0][0],
        pes.qhg_zero[0][0],
    ]
    ...
    for i, path in enumerate(pes.path):
        for j, e_abs in enumerate(pes.e_abs[i]):
            ...
            formatted_list = [constants.KCAL_TO_AU * x for x in relative]
