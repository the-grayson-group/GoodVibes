...
class calcD3:
   def __init__(self, fileData, functional, s6, rs6, s8, a1, a2, damp, abc, intermolecular, pairwise, verbose):
      ...
      if hasattr(fileData, "CARTESIANS"):
          ## Arrays for atoms and Cartesian coordinates ##
      ...
      for j in range(0,natom):
         ...
         for k in range(j+1,natom):
            ...
            if k>j:
               ...
               if nonbonded == True and totdist < RADII[atomtype[j]]+RADII[atomtype[k]]: scalefactor = 0
               ...
               if pairwise == True and scalefactor != 0 and -1 < (self.attractive_r6_term + self.attractive_r8_term) < -0.4:
                   total += self.attractive_r6_term + self.attractive_r8_term
                   print("   --- Pairwise interaction between atoms", (j+1), "and", (k+1),": Edisp =", "%.6f" % (self.attractive_r6_term + self.attractive_r8_term), "kcal/mol", total)
