...
class Gaussian(logfileparser.Logfile):
    ...
    def extract(self, inputfile, line):
        ...
        if not hasattr(self, "symmno"):
            self.symmno = 1

        if not hasattr(self, "linear_mol"):
            self.linear_mol = False
        ...
        if "DeltaG (solv)" in line.strip():
            self.dgsolv = float(line.split()[4])
        ...
        if line.strip().startswith('Full point group'):
            if line.strip().split()[3] == 'D*H' or line.strip().split()[3] == 'C*V':
                self.linear_mol = True
            self.point_group = line.strip().split()[3]

        if line.strip().startswith('Rotational constants (GHZ):'):
            try:
        ...
        if not hasattr(self, 'solvation'):
            self.solvation = ['gas']

        if "Model                : PCM" in line: self.solvation = ['PCM']
        if "Model                : CPCM" in line: self.solvation = ['CPCM']
        if "Atomic radii         : SMD-Coulomb" in line: self.solvation = ['SMD']
