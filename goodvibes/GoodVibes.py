...
from .bbe import calc_bbe

# Importing regardless of relative import
try:
    from .vib_scale_factors import (
        scaling_data_dict,
        scaling_data_dict_mod,
        scaling_refs,
    )
except ImportError:
...
def get_selectivity(pattern, files, boltz_facs, boltz_sum, temperature, log, dup_list):
    ...
    if len(a_files) is 0 or len(b_files) is 0:
        ...
        sys.exit(
            "   Please edit either your filenames or selectivity pattern argument and try again\n"
        )
    # Grab Boltzmann sums
    for file in files:
        duplicate = False
    ...
    try:
        dd_free_energy = (
            ...
            / constants.J_TO_AU
            * temperature
            * math.log((50 + abs(ee) / 2.0) / (50 - abs(ee) / 2.0))
...
def get_boltz(files, thermo_data, clustering, clusters, temperature, dup_list):
    boltz_facs, weighted_free_energy, e_rel, e_min, boltz_sum = (
        {},
        {},
        {},
        sys.float_info.max,
...
def check_dup(files, thermo_data):
    ...
    for i, file in enumerate(files):
        for j in range(0, i):
            ...
            if hasattr(bbe_i, "roconst") and hasattr(bbe_j, "roconst"):
                if len(bbe_i.roconst) == len(bbe_j.roconst):
                    ro_diff = np.linalg.norm(
                        np.array(bbe_i.roconst) - np.array(bbe_j.roconst)
                    )
            if hasattr(bbe_i, "frequency_wn") and hasattr(bbe_j, "frequency_wn"):
                if (
                    len(bbe_i.frequency_wn) == len(bbe_j.frequency_wn)
                    and len(bbe_i.frequency_wn) > 0
                ):
                    freq_diff = [
                        np.linalg.norm(freqi - freqj)
                        for freqi, freqj in zip(bbe_i.frequency_wn, bbe_j.frequency_wn)
                    ]
            ...
            if (
                e_diff < e_cutoff
                and ro_diff < ro_cutoff
                and mae_freq_diff < mae_freq_cutoff
                and max_freq_diff < max_freq_cutoff
            ):
...
def main():
    ...
    if options.xyz:
        xyz = utils.xyz_out("Goodvibes", "xyz", "output")
    # If user has specified different file extensions
    if options.custom_ext or os.environ.get("GOODVIBES_CUSTOM_EXT", ""):
        custom_extensions = options.custom_ext.split(",") + os.environ.get(
            "GOODVIBES_CUSTOM_EXT", ""
        ).split(",")
        for ext in custom_extensions:
            constants.SUPPORTED_EXTENSIONS.add(ext.strip())
    ...
    log = utils.Logger("Goodvibes", options.output, options.csv)
                ...
                for file in glob(elem):
                    if options.spc is False or options.spc is "link":
                        ...
                        if file.find("_" + options.spc + ".") == -1:
                            ...
                            if (
                                not (
                                    os.path.exists(name + "_" + options.spc + ".log")
                                    or os.path.exists(name + "_" + options.spc + ".out")
                                )
                                and options.spc != "link"
                            ):
                                sys.exit(
    ...
    if options.conc:
        ...
        gas_phase = True
        options.conc = constants.ATMOS / (constants.GAS_CONSTANT * options.temperature)
    ...
    for i, key in enumerate(files):
        if progress[key] == "Error":
            ...
            log.write(
                ...
                "running. This file will be omitted from further calculations.".format(
                    key
                )
            )
            remove_key.append([i, key])
    ...
    if options.freq_scale_factor is not False:
        ...
        if utils.all_same(l_o_t):
            ...
            utils.print_check_fails(
                log,
                filtered_calcs_l_o_t[1],
                filtered_calcs_l_o_t[0],
                "levels of theory",
            )

    # Exit program if a comparison of Boltzmann factors is requested and level of theory is not uniform across all files
    if not utils.all_same(l_o_t) and (
        options.boltz is not False or options.ee is not False
    ):
        sys.exit(
    ...
    if options.mm_freq_scale_factor is not False:
        if utils.all_same(l_o_t) and "ONIOM" in l_o_t[0]:
            log.write(
                "\n\n   User-defined vibrational scale factor "
                + str(options.mm_freq_scale_factor)
                + " for MM region of "
                + l_o_t[0]
            )
            log.write(f"\n   REF: {oniom_scale_ref}")
    ...
    if freespace != 1000.0:
        log.write(
            ...
            + options.freespace
            + ": free volume "
            + str("%.3f" % (freespace / 10.0))
            + " (mol/l) corrects the translational entropy"
        )
    ...
    if options.cosmo_int:
        ...
        t_interval, gsolv_dicts = parse.cosmo_rs_out(cfile, files, interval=cinterval)
        options.temperature_interval = True

    elif options.cosmo is not False:  # Read from COSMO-RS output
        try:
            cosmo_solv = parse.cosmo_rs_out(options.cosmo, files)
            log.write("\n\n   Reading COSMO-RS file: " + options.cosmo)
        except ValueError:
            cosmo_solv = None
            log.write(
                "\n\n   Warning! COSMO-RS file "
                + options.cosmo
    ...
    log.write(
        "\n\n   Entropic quasi-harmonic treatment: frequency cut-off value of "
        + str(options.S_freq_cutoff)
        + " wavenumbers will be applied."
    )
    if options.QS == "grimme":
        ...
        log.fatal(
            "\n   FATAL ERROR: Unknown quasi-harmonic model "
            + options.QS
    ...
    if options.QH:
        log.write(
            "\n\n   Enthalpy quasi-harmonic treatment: frequency cut-off value of "
            + str(options.H_freq_cutoff)
            + " wavenumbers will be applied."
        )
        log.write(
    ...
    for file in files:
        ...
        if options.D3 or options.D3BJ:
            ...
            functional = parse.level_of_theory(file).split("/")[0]
            if options.D3:
                damp = "zero"
            elif options.D3BJ:
                damp = "bj"
            ...
            try:
                ...
                d3_energy = (
                    d3_calc.attractive_r6_vdw
                    + d3_calc.attractive_r8_vdw
                    + d3_calc.repulsive_abc
                ) / constants.KCAL_TO_AU
            except:
        ...
        if options.media != False:
            try:
                ...
                from media import solvents
            if (
                options.media.lower() in solvents
        ...
        bbe = calc_bbe(
            ...
            options.QS,
            options.QH,
            options.S_freq_cutoff,
            options.H_freq_cutoff,
            options.temperature,
            conc,
            ...
            cosmo=cosmo_option,
            ssymm=ssymm_option,
            mm_freq_scale_factor=vmm_option,
        )
    ...
    thermo_data = dict(
        zip(file_list, bbe_vals)
    )  # The collected thermochemical data for all files
            ...
            if len(dup_list) != 0:
                for dup in dup_list:
                    if dup[0] == file:
                        duplicate = True
                        log.write(
                            "\nx  {} is a duplicate or enantiomer of {}".format(
                                dup[0].rsplit(".", 1)[0], dup[1].rsplit(".", 1)[0]
                            )
                        )
                        break
            if not duplicate:
                ...
                if bbe.linear_warning:
                    ...
                    if hasattr(bbe, "scf_energy") and not hasattr(
                    ...
                    if not hasattr(bbe, "gibbs_free_energy"):
                        ...
                        if (
                            options.media is not False
                            and options.media.lower() in solvents
                            and options.media.lower()
                            == os.path.splitext(os.path.basename(file))[0].lower()
                        ):
                ...
                if options.cosmo and cosmo_solv is not None:
                    log.write(
                        "{:13.6f} {:16.6f}".format(
                            cosmo_solv[file],
                            bbe.qh_gibbs_free_energy + cosmo_solv[file],
                        )
                    )
                if options.boltz is True:
                    log.write(
                            ...
                            if id == len(cluster) - 1:
                                ...
                                log.write(
                                    ...
                                    + "{name:<{var_width}} {gval:13.6f} {weight:6.2f}".format(
                                        name="Boltzmann-weighted Cluster "
                                        + alphabet[n].upper(),
                                        var_width=len(stars) - 24,
                                        gval=weighted_free_energy[
                                            "cluster-" + alphabet[n].upper()
                                        ]
                                        / boltz_facs["cluster-" + alphabet[n].upper()],
                                        weight=100
                                        * boltz_facs["cluster-" + alphabet[n].upper()]
                                        / boltz_sum,
                                    ),
    ...
    if options.check:
        utils.check_files(
            log, files, thermo_data, options, stars, l_o_t, orientation, grid
        )
    ...
    # Running a variable temperature analysis of the enthalpy, entropy and the free energy
        ...
        if options.cosmo_int is False:
            temperature_interval = [
                float(temp) for temp in options.temperature_interval.split(",")
            ]
            # If no temperature step was defined, divide the region into 10
            if len(temperature_interval) == 2:
                temperature_interval.append(
                    (temperature_interval[1] - temperature_interval[0]) / 10.0
                )
            interval = range(
        ...
        for h, file in enumerate(files):  # Temperature interval
            ...
            for i in range(len(interval)):  # Iterate through the temperature range
                temp = interval[i]
                if gas_phase:
                    conc = constants.ATMOS / constants.GAS_CONSTANT / temp
                else:
                    conc = options.conc
                linear_warning = []
                if options.cosmo_int is False:
                ...
                if linear_warning == [
                    ...
                    if not hasattr(bbe, "gibbs_free_energy"):
                        ...
                        log.write(
                            "{:<39} {:13.1f}".format(
                                os.path.splitext(os.path.basename(file))[0], temp
                            ),
                            thermodata=True,
                        )
                        ...
                        if all(
                            ...
                            if options.QH:
                                if options.cosmo_int:
                                    log.write(
                                        " {:24.6f} {:13.6f} {:10.6f} {:10.6f} {:13.6f} {:13.6f}".format(
                                            bbe.enthalpy,
                                            bbe.qh_enthalpy,
                                            (temp * bbe.entropy),
                                            (temp * bbe.qh_entropy),
                                ...
                                if options.cosmo_int:
                                    log.write(
                                        " {:24.6f} {:10.6f} {:10.6f} {:13.6f} {:13.6f}".format(
                                            bbe.enthalpy,
                                            (temp * bbe.entropy),
                                            (temp * bbe.qh_entropy),
                                            bbe.gibbs_free_energy,
    ...
    if options.cputime:
        log.write(
            ...
            "{:>4}\n".format(
                "TOTAL CPU",
                total_cpu_time.day + add_days - 1,
                "days",
                total_cpu_time.hour,
                "hrs",
                total_cpu_time.minute,
                "mins",
                total_cpu_time.second,
                "secs",
            )
        )
                ...
                for k, path in enumerate(pes.path):
                    if options.QH:
                        zero_vals = [
                            ...
                            pes.h_zero[k][0],
                            pes.qh_zero[k][0],
                            temp * pes.ts_zero[k][0],
                            temp * pes.qhts_zero[k][0],
                            pes.g_zero[k][0],
                            pes.qhg_zero[k][0],
                        ]
                    else:
                    ...
                    if options.spc is False:
                        ...
                        if options.QH and options.cosmo_int:
                            log.write(
                                "{:>13} {:>13} {:>10} {:>13} {:>13} {:>10} {:>10} {:>14} {:>14} {:>14}".format(
                                    " DE_SPC",
                                    "DE",
                                    "DZPE",
                                    "DH_SPC",
                    ...
                    for l, e_abs in enumerate(pes.e_abs[k]):
                        ...
                        if pes.units == "kJ/mol":
                            formatted_list = [
                                constants.J_TO_AU / 1000.0 * x for x in relative
                            ]
                        else:
                            formatted_list = [
                                constants.KCAL_TO_AU * x for x in relative
                            ]  # Defaults to kcal/mol
                        log.write("\no  ")
                        if options.spc is False:
                            formatted_list = formatted_list[1:]
                            format_1 = (
                                "{:<39} {:13.1f} {:10.1f} {:13.1f} {:13.1f} {:10.1f} {:10.1f} {:13.1f} "
                                "{:13.1f} {:13.1f}"
                            )
                            format_2 = (
                                "{:<39} {:13.2f} {:10.2f} {:13.2f} {:13.2f} {:10.2f} {:10.2f} {:13.2f} "
                                "{:13.2f} {:13.2f}"
                            )
                            if options.QH and options.cosmo_int:
                                if pes.dec == 1:
                                    log.write(
                                        format_1.format(
                                            pes.species[k][l], *formatted_list
                                        ),
                                        thermodata=True,
                                    )
                                if pes.dec == 2:
                            ...
                            if options.QH and options.cosmo_int:
                                if pes.dec == 1:
                                    log.write(
                                        "{:<39} {:13.1f} {:13.1f} {:10.1f} {:13.1f} {:13.1f} {:10.1f} {:10.1f} "
                                        "{:13.1f} {:13.1f} {:13.1f}".format(
                                            pes.species[k][l], *formatted_list
                                        ),
                                        thermodata=True,
                                    )
                        ...
                        if pes.boltz:
                            ...
                            log.write(
                                "\n  "
                                + "{:<39} {:13.2f}%{:24.2f}%{:35.2f}%{:13.2f}%".format(
                                    "", *selectivity
                                )
                            )
                            sels.append(selectivity)
                        formatted_list = [
                            round(formatted_list[x], 6)
                            for x in range(len(formatted_list))
                        ]
                    if pes.boltz == "ee" and len(sels) == 2:
                        ...
                        if options.spc is False:
                            log.write(
                                ...
                                + stars
                                + "\n   "
                                + "{:<39} {:13.1f}%{:24.1f}%{:35.1f}%{:13.1f}%".format(
                                    "ee (%)", *ee
                                )
                            )
                        else:
                            log.write(
                                ...
                                + stars
                                + "\n   "
                                + "{:<39} {:27.1f} {:24.1f} {:35.1f} {:13.1f} ".format(
                                    "ee (%)", *ee
                                )
                            )
                    log.write("\n" + stars + "\n")
            ...
            for i, path in enumerate(pes.path):
                if options.QH:
                    zero_vals = [
                        ...
                        pes.h_zero[i][0],
                        pes.qh_zero[i][0],
                        options.temperature * pes.ts_zero[i][0],
                        options.temperature * pes.qhts_zero[i][0],
                        pes.g_zero[i][0],
                        pes.qhg_zero[i][0],
                    ]
                else:
                ...
                if pes.boltz:
                    e_sum, h_sum, g_sum, qhg_sum, cosmo_qhg_sum = (
                        0.0,
                        0.0,
                        0.0,
                        0.0,
                    ...
                    for j, e_abs in enumerate(pes.e_abs[i]):
                        if options.QH:
                            species = [
                                ...
                                pes.h_abs[i][j],
                                pes.qh_abs[i][j],
                                options.temperature * pes.s_abs[i][j],
                                options.temperature * pes.qs_abs[i][j],
                                pes.g_abs[i][j],
                                pes.qhg_abs[i][j],
                            ]
                        else:
                ...
                for j, e_abs in enumerate(pes.e_abs[i]):
                    ...
                    if options.spc is False:
                        ...
                        if options.QH and options.cosmo:
                            if pes.dec == 1:
                                log.write(
                                    "{:<39} {:13.1f} {:10.1f} {:13.1f} {:13.1f} {:10.1f} {:10.1f} {:13.1f} "
                                    "{:13.1f} {:13.1f}".format(
                                        pes.species[i][j], *formatted_list
                                    ),
                                    thermodata=True,
                                )
                            if pes.dec == 2:
                            ...
                            if pes.dec == 1:
                                log.write(
                                    "{:<39} {:13.1f} {:10.1f} {:13.1f} {:10.1f} {:10.1f} {:13.1f} "
                                    "{:13.1f}".format(
                                        pes.species[i][j], *formatted_list
                                    ),
                                    thermodata=True,
                            ...
                            if pes.dec == 2:
                                log.write(
                                    "{:<39} {:13.2f} {:10.2f} {:13.2f} {:10.2f} {:10.2f} {:13.2f} "
                                    "{:13.2f}".format(
                                        pes.species[i][j], *formatted_list
                                    ),
                                    thermodata=True,
                        ...
                        if options.QH and options.cosmo:
                            ...
                            if pes.dec == 2:
                                log.write(
                                    "{:<39} {:13.1f} {:13.2f} {:10.2f} {:13.2f} {:13.2f} {:10.2f} {:10.2f} "
                            ...
                            if pes.dec == 1:
                                log.write(
                                    "{:<39} {:13.1f} {:13.1f} {:10.1f} {:13.1f} {:10.1f} {:10.1f} {:13.1f} "
                                    "{:13.1f}".format(
                                        pes.species[i][j], *formatted_list
                                    ),
                                    thermodata=True,
                            ...
                            if pes.dec == 2:
                                log.write(
                                    "{:<39} {:13.2f} {:13.2f} {:10.2f} {:13.2f} {:10.2f} {:10.2f} {:13.2f} "
                                    "{:13.2f}".format(
                                        pes.species[i][j], *formatted_list
                                    ),
                                    thermodata=True,
                    ...
                    if pes.boltz:
                        ...
                        sels.append(selectivity)
                    formatted_list = [
                        round(formatted_list[x], 6) for x in range(len(formatted_list))
                    ]
                if pes.boltz == "ee" and len(sels) == 2:
                    ee = [sels[0][x] - sels[1][x] for x in range(len(sels[0]))]
                    if options.spc is False:
    ...
    if options.ee is not False:
        ...
        ee, er, ratio, dd_free_energy, failed, preference = get_selectivity(
            options.ee, files, boltz_facs, boltz_sum, options.temperature, log, dup_list
        )
        if not failed:
            log.write(
                "\n   "
                + "{:<39} {:>13} {:>13} {:>13} {:>13} {:>13}".format(
                    "Selectivity",
                    "Excess (%)",
                    "Ratio (%)",
                    "Ratio",
            ...
            log.write("\n" + selec_stars)
            log.write(
                "\no {:<40} {:13.2f} {:>13} {:>13} {:>13} {:13.2f}".format(
                    "", ee, er, ratio, preference, dd_free_energy
                ),
                thermodata=True,
            )
            log.write("\n" + selec_stars + "\n")
    ...
    if options.graph is not False:
        ...
        plotting.graph_reaction_profile(graph_data, log, options, plt)
